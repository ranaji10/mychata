...
export const Route = createFileRoute("/manual")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Bilingual instructions and house information for your cottage.",
      },
...
function ManualPage() {
  ...
  const { data: sections, isLoading } = useQuery({
    queryKey: ["manual-sections", property?.id],
    enabled: !!property,
    queryFn: async () => {
  ...
  const { data: feedback } = useQuery({
    queryKey: ["manual-feedback", property?.id, sections?.length],
    enabled: !!property && isAdmin && !!sections,
    queryFn: async () => {
  ...
  const resolveFeedback = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["manual-feedback", property?.id] });
      toast.success(t("Upozornění vyřešeno.", "Report resolved."));
    },
  });
  ...
  const addSection = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("manual_sections").insert({
        ...
        title_cs: form.titleCs,
        title_en: form.titleEn,
        content_cs: form.contentCs,
  ...
  return (
    <AppShell>
      <PageHeader
        title={t("Manuál chaty", "House Manual")}
        ...
        action={
          isAdmin ? (
            <button
              ...
              aria-label={t("Přidat sekci", "Add section")}
      ...
      <section className="card mb-4 p-4">
        <h2 className="flex items-center gap-2 font-bold">
          ...
          {t("Zeptejte se manuálu", "Ask the manual")}
        </h2>
        <div className="mt-2 flex gap-2">
          <input
            className="field flex-1"
            placeholder={t("Např. Jak se pouští topení?", "E.g. How do I turn on the heating?")}
            value={question}
            onChange={(e) => setQuestion(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && ask()}
          />
          <button className="btn-primary shrink-0" disabled={asking} onClick={ask}>
            {asking ? "…" : t("Zeptat se", "Ask")}
          </button>
        </div>
        {answer && (
          <p className="mt-3 whitespace-pre-wrap rounded-2xl bg-secondary p-3 text-[15px]">
            {answer}
          </p>
        )}
      </section>
      ...
      {property && (
        <section className="card mb-4 flex items-center gap-4 p-4">
          <div className="rounded-xl bg-card p-1 ring-1 ring-border">
            <QRCodeSVG value={publicUrl} size={82} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-bold">{t("Veřejný manuál", "Public manual")}</p>
            <p className="text-[13px] text-muted-foreground">
              {t("Hosté jej otevřou bez přihlášení.", "Guests can open it without signing in.")}
            </p>
          </div>
          <button
            onClick={async () => {
            ...
            aria-label={t("Kopírovat odkaz", "Copy link")}
      ...
      {isAdmin && !!feedback?.length && (
        <section className="mb-4 rounded-2xl bg-warn-soft p-4">
          <div className="flex items-center gap-2">
            <Flag className="size-5 text-warn" />
            <h2 className="font-bold">
              {t("Nahlášené neaktuální informace", "Outdated information reports")}
            </h2>
          </div>
          <div className="mt-3 space-y-2">
            {feedback.map((item) => (
              <div key={item.id} className="flex items-center gap-3 rounded-xl bg-card p-3">
                <p className="flex-1 text-[14px] font-semibold">
                  {
                    sections?.find((section) => section.id === item.section_id)?.[
                      lang === "en" ? "title_en" : "title_cs"
                    ]
                  }
                </p>
                <button
                  onClick={() => resolveFeedback.mutate(item.id)}
                  className="grid size-11 place-items-center rounded-xl bg-ok-soft text-ok"
                  aria-label={t("Označit jako vyřešené", "Mark resolved")}
                >
      ...
      {isLoading ? (
        ...
        <EmptyState icon={BookOpen} title={t("Manuál je zatím prázdný.", "The manual is empty.")} />
      ) : (
        ...
        <div className="fixed inset-0 z-50 grid place-items-end bg-foreground/35 p-4 sm:place-items-center">
          ...
          >
            ...
            <input
              required
              value={form.titleCs}
              onChange={(e) => setForm({ ...form, titleCs: e.target.value })}
              className="field"
              placeholder="Název česky"
            />
            <textarea
              required
              value={form.contentCs}
              onChange={(e) => setForm({ ...form, contentCs: e.target.value })}
              className="field min-h-24"
              placeholder="Obsah česky"
            />
            <input
              required
              value={form.titleEn}
