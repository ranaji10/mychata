...
function HandoverPage() {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `mychata.handover.${property.id}`,
      JSON.stringify({ checked, note, issue }),
    );
  }, [property?.id, checked, note, issue]);

  const { data: activeBooking } = useQuery({
    queryKey: ["handover-booking", property?.id, currentMember?.id],
    enabled: !!property && !!currentMember,
    queryFn: async () => {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (photo && property) {
        const path = `${property.id}/handovers/${crypto.randomUUID()}-${photo.name.replace(/[^a-zA-Z0-9._-]/g, "-")}`;
        const { error } = await supabase.storage.from("my-chata-files").upload(path, photo);
        if (error) throw error;
        photoUrl = path;
      }
      ...
      if (issue && property && handover) {
        ...
        const { error: issueError } = await supabase.from("handover_issues").insert({
  ...
  const memberName = (id: string | null) =>
    members.find((m) => m.id === id)?.name ?? t("Neznámý", "Unknown");
  ...
  return (
    <AppShell>
      ...
      <section className="card p-4">
        ...
        <div className="mt-3 space-y-1">
          {items.map((item, i) => (
            ...
            >
              ...
              <span
                className={`text-[15px] font-semibold ${checked[i] ? "text-muted-foreground line-through" : ""}`}
              >
                {item}
              </span>
            </button>
        ...
        <div className="mt-3">
          <label htmlFor="handover-note" className="mb-1 block text-[13px] font-bold">
        ...
        <div className="mt-3">
          <label
            htmlFor="handover-issue"
            className="mb-1 flex items-center gap-2 text-[13px] font-bold"
          >
            <Wrench className="size-4" />
            {t("Nahlásit závadu (nepovinné)", "Report an issue (optional)")}
          </label>
          <input
            id="handover-issue"
            value={issue}
            ...
            className="field"
            placeholder={t("Např. protékající kohoutek", "E.g. leaking tap")}
          />
        </div>
        <label className="btn-secondary mt-3 w-full cursor-pointer">
          <Camera className="size-5" />
          {photo ? photo.name : t("Přidat kontrolní fotografii", "Add inspection photo")}
          <input
            type="file"
