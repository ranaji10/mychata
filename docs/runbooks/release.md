...
5. Smoke test on mychata.cz in a real browser with the developer console open (no red errors; see B-002): sign in, open Home, calendar, manual; open one public link in a private window. With CI: run the workflow manually with `e2e_base_url = https://mychata.cz`.
